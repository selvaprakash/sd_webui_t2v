...
def t2v_api(_, app: FastAPI):
    ...
    @app.post("/t2v/run")
    async def t2v_run(prompt: str,passkey: str, n_prompt: Union[str, None] = None, steps: Union[int, None] = None, frames: Union[int, None] = None, seed: Union[int, None] = None, \
        ...
        if passkey== 'qwerty0987':
            pass
        else:
            return JSONResponse(
                status_code=500,
                content={"detail": "Passkey is Wrong."},
            ) 
        for basedir in basedirs:
            sys.path.extend([
                basedir + '/scripts',
